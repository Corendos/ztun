...
pub const AuthenticationKey = struct {
    buffer: []const u8,
    value: []const u8,

    pub fn deinit(self: AuthenticationKey, allocator: std.mem.Allocator) void {
...
pub const ShortTermAuthentication = struct {
    ...
    pub fn computeKey(self: ShortTermAuthentication, out: []u8) !AuthenticationKey {
        var stream = std.io.fixedBufferStream(out);
        try io.writeOpaqueString(self.password, stream.writer());
        return AuthenticationKey{ .buffer = out, .value = stream.getWritten() };
    }

    pub fn computeKeyAlloc(self: ShortTermAuthentication, allocator: std.mem.Allocator) !AuthenticationKey {
...
pub const LongTermAuthentication = struct {
    ...
    pub fn computeKey(self: LongTermAuthentication, out: []u8) !AuthenticationKey {
        ...
        return AuthenticationKey{ .buffer = out, .value = out[0..std.crypto.hash.Md5.digest_length] };
    }

    pub fn computeKeyAlloc(self: LongTermAuthentication, allocator: std.mem.Allocator) !AuthenticationKey {
...
pub const Authentication = union(AuthenticationType) {
    ...
    pub fn computeKey(self: Authentication, out: []u8) !AuthenticationKey {
        return switch (self) {
            inline else => |auth| auth.computeKey(out),
        };
    }

    pub fn computeKeyAlloc(self: Authentication, allocator: std.mem.Allocator) !AuthenticationKey {
        return switch (self) {
...
test "compute short-term authentication key" {
    ...
    try std.testing.expectEqualSlices(u8, true_key, key.value);
}
